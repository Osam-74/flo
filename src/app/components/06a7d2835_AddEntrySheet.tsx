...
export function AddEntrySheet({ open, onClose, people, currency, onSave, initialType = 'income' }: Props) {
  ...
  // When the soft keyboard dismisses on mobile, the visual viewport resizes back
  // but vaul's drawer may not reflow. We force a translate reset when keyboard closes.
  ...
  useEffect(() => {
    ...
    const onViewportChange = () => {
      ...
      const prevHeight = lastVVHeight.current;

      // Keyboard just opened: viewport shrank significantly
      if (currentHeight < prevHeight - 100) {
        isKeyboardOpen.current = true;
      }
      // Keyboard just closed: viewport height increased significantly
      else if (currentHeight > prevHeight + 100 && isKeyboardOpen.current) {
        isKeyboardOpen.current = false;
        // Blur active input first to prevent re-trigger
        if (document.activeElement instanceof HTMLElement) {
          document.activeElement.blur();
        }
        // Force drawer back to its natural bottom position
        if (drawerContentRef.current) {
          drawerContentRef.current.style.transform = 'translateY(0px)';
          drawerContentRef.current.style.transition = 'transform 0.35s cubic-bezier(0.32,0.72,0,1)';
          // Clear transform after animation completes so vaul can take control again
          setTimeout(() => {
            if (drawerContentRef.current) {
        ...
        // Force window scroll back to top so page doesn't stay scrolled
        window.scrollTo({ top: 0, behavior: 'instant' as ScrollBehavior });
      }
      lastVVHeight.current = currentHeight;
    };
...
const card: React.CSSProperties = {
  ...
  borderRadius: 16,
  boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 4px 16px rgba(0,119,182,0.08)',
