...
export function EditModal({ open, tx, people, onClose, onSave }: EditModalProps) {
  ...
  useEffect(() => {
    if (!tx) return;
    setAmount(String(tx.type === 'credit' ? (tx.creditPaid || 0) : (tx.amount || 0)));
    setDate(tx.date || '');
    setDesc(tx.desc || '');
    setNote(tx.note || '');
    setPerson(tx.person || '');
  ...
  const save = () => {
    ...
    if (amt <= 0 && tx?.type !== 'credit') { toast.error('Enter a valid amount'); return; }
    if (!tx) return;
    const updates: Partial<Transaction> = { date, desc: desc || tx.desc, note, person, cat };
    if (tx.type === 'credit') Object.assign(updates, { creditPaid: amt, amount: amt });
    else Object.assign(updates, { amount: amt });
  ...
  return (
    <BottomSheet open={open} onClose={onClose}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <Field label="Amount"><input style={inp} type="number" min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} /></Field>
